...
public class Var {
    ...
    public static String ozNull(OzDate dat) {
        if (dat == null || dat.isNull()) {
            return "";
        } else {
            return dat.toOz();
        }
    ...
    public static String ozNullShow(OzDate dat) {
        if (dat == null || dat.isNull()) {
            return "";
        } else {
            return dat.toShow();
    ...
    public static boolean notZeroNull(BigDecimal num) {
        if (num == null) {
            return false;
        }
        if (num.compareTo(new BigDecimal("0.00")) == 0) {
    ...
    public static boolean hasDatesInRange(OzDate testDate, int testNights, OzDate rangeDate,
            int rangeNights) {
        OzDate testEndDate = new OzDate(testDate.addDays(testNights - 1));
        OzDate rangeEndDate = new OzDate(rangeDate.addDays(rangeNights - 1));
        if (testDate.daysBefore(rangeDate) > 0 || rangeEndDate.daysBefore(testEndDate) > 0) {
            return false;
        } else {
            return true;
        }
    ...
    public static boolean isBeforeDeadline(OzDate basisDate, int endDays, int endHour) {
        OzDate today = new OzDate(0);
        if (basisDate.daysBefore(today) > endDays) {
            return false;
        } else if (basisDate.daysBefore(today) < endDays) {
            return true;
        } else { // daysbefore == endDays
            if (today.getHour() < endHour) {
                return true;
            } else {
                return false;
            }
